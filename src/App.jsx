...
function ChromeV({ go }) { return <div><Tags rs={["ux", "service", "pm"]} /><H2>Chrome — scanning live products</H2>
  ...
  <P><strong>Service Designers</strong> use Chrome at the start of a project to understand the current live experience, and after delivery to verify the product matches what was designed. <strong>UX Designers</strong> use it to inform prototypes — scanning the real product so Claude can generate an exact replica with new features. <strong>Product Managers</strong> use it for competitor analysis. <strong>Content Designers</strong> use it to audit live copy against approved content.</P>
...
const VM = { intro: Intro, chrome: ChromeV, figma: FigmaV, figjam: FigJamV, proto: ProtoV, code: CodeV, ds: DSV, jm: JMV, slack: SlackV, cost: CostV, bench: BenchV, tools: ToolsV, next: NextV };
